...
if(isset($datos["idPerfil"], $datos["nombre"], $datos["permisos"])){
    $id = $datos["idPerfil"];
    $nombre = $datos["nombre"];
    ...
    $sql = "UPDATE perfiles SET NombrePerfil = ?, Permisos = ? WHERE IdPerfil = ?";
    $stmt = mysqli_prepare($link, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ssi", $nombre, $permisos, $id);
    ...
    $Resultado = "error: Los campos id, nombre y permisos son requeridos";
}
